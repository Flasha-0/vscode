...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(async (msg) => {
      switch (msg.type) {
        case 'sendMessage':
          vscode.window.showInformationMessage(`[${msg.mode}] ${msg.text}`);
          break;
        case 'setMode':
...
.mode-btn{padding:3px 10px;border-radius:5px;border:1px solid #30363D;background:#161B22;color:#8B949E;cursor:pointer;font-size:11px;white-space:nowrap;transition:.2s}
...
.model-picker{margin-bottom:6px;font-size:11px;color:#8B949E}
select{background:#161B22;color:#E6EDF3;border:1px solid #30363D;border-radius:4px;padding:2px 5px;font-size:11px}
</style></head><body>
<div class="model-picker"><select id="model">
<option value="opencode/big-pickle">Big Pickle ðŸ¥’</option>
<option value="opencode/deepseek-v4-flash-free">DeepSeek Flash âš¡</option>
</select></div>
